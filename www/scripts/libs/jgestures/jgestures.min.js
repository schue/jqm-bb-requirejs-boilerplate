 ...
 */
 ...
 (function($) {
    ...
    */
    ...
    $.jGestures.defaults.thresholdMove = 10;
    $.jGestures.defaults.thresholdSwipe = 20;
    $.jGestures.defaults.thresholdSwipeDirection = 50; // JIM MODIFICATION
    $.jGestures.defaults.thresholdHoldDuration = 2000; // JIM MODIFICATION
    $.jGestures.data = {};
...
//          event_.currentTarget.removeEventListener('mouseup', _onTouchend, false);
        ...
        for (_sType in _oDatajQueryGestures) {
            ...
            switch(_sType) {
                ...
                case 'swipeleft':
                case 'swipeleftup':
                    if ( /* _bHasTouches && */ _bHasMoved === true && _bHasSwipeGesture===true) {  // JIM MODIFICATION
                        _bIsSwipe = true;
                        _oDetails.type = [
                            'swipe',
                            ((_oDetails.delta[0].lastX != 0 && Math.abs(_oDetails.delta[0].lastX) > $.jGestures.defaults.thresholdSwipeDirection) ? ((_oDetails.delta[0].lastX > 0) ? 'right' : 'left') : ''),  // JIM MODIFICATION
                            ((_oDetails.delta[0].lastY != 0 && Math.abs(_oDetails.delta[0].lastY) > $.jGestures.defaults.thresholdSwipeDirection) ? ((_oDetails.delta[0].lastY > 0) ? 'down' : 'up') : '')  // JIM MODIFICATION
                        ].join('');
                        /* _$element.triggerHandler( */ jQuery(event_.target).trigger(_oDetails.type, _oDetails); // JIM MODIFICATION
                    }
                break; 
...
//          _oObj.hasTouchmoved = false;
    ...
    */      
            ...
            switch(_sType) {
                ...
                case 'pinchclose': 
                    ...
                    if (( _iDelta < 1 ) && (_iDelta % 1) < (1 - $.jGestures.defaults.thresholdPinchclose)) {
                        /* _$element.triggerHandler( */ jQuery(event_.target).trigger('pinchclose', _createOptions ({type: 'pinchclose', scale:_iDelta, vector: -1, touches: null, startMove: _oDatajQueryGestures.oStartTouch, event:event_, timestamp:_oDatajQueryGestures.oStartTouch.timestamp,description: 'pinch:-1:close' }) ); // JIM MODIFICATION
                    }
                break;
                ...
                case 'pinchopen':
                    ...
                    if ( ( _iDelta > 1 ) && (_iDelta % 1) > ($.jGestures.defaults.thresholdPinchopen) ) {
                        /* _$element.triggerHandler( */ jQuery(event_.target).trigger('pinchopen', _createOptions ({type: 'pinchopen', scale:_iDelta, vector: +1, touches: null, startMove: _oDatajQueryGestures.oStartTouch, event:event_, timestamp:_oDatajQueryGestures.oStartTouch.timestamp,description: 'pinch:+1:open'}) ); // JIM MODIFICATION
                    }
                break;
                ...
                case 'rotatecw':
                    ...
                    if ( ( _iDelta > 1 ) && (_iDelta  > $.jGestures.defaults.thresholdRotatecw) ) {
                        /* _$element.triggerHandler( */ jQuery(event_.target).trigger('rotatecw', _createOptions ({type: 'rotatecw', rotation:_iDelta, vector: +1, touches: null, startMove: _oDatajQueryGestures.oStartTouch, event:event_, timestamp:_oDatajQueryGestures.oStartTouch.timestamp,description: 'rotate:+1:clockwise'}) ); // JIM MODIFICATION
                    }
                break;
                ...
                case 'rotateccw':
                    ...
                    if ( ( _iDelta < 1 ) &&  ( -1*(_iDelta) > $.jGestures.defaults.thresholdRotateccw ) ) {
                        /* _$element.triggerHandler( */ jQuery(event_.target).trigger('rotateccw', _createOptions ({type: 'rotateccw', rotation:_iDelta, vector: -1, touches: null, startMove: _oDatajQueryGestures.oStartTouch, event:event_, timestamp:_oDatajQueryGestures.oStartTouch.timestamp,description: 'rotate:-1:counterclockwise'}) ); // JIM MODIFICATION
                    }
                break;
    ...
    /* START JIM MODIFICATION: add taphold */
    jQuery.event.special.taphold = {
      setup: function() {
        var thisObject = this, $this = jQuery(this);
        $this.bind("touchstart mousedown", function(event) {
          ...
          timer = setTimeout(function() {
            jQuery(event.target).trigger("taphold", jQuery.Event("taphold", {target: event.target}));
          }, 750);
        });
      }
    };
